...
public class HlsService
{
    ...
    {
        var bitRateStr = bitRates.Length > 0 ? bitRates[0].ToString() : "default"; // 蜊倅ｸ繝薙ャ繝医Ξ繝ｼ繝亥燕謠・
    ...
    public Task StartCacheCleanupAsync()
    {
        ...
        // 繝舌ャ繧ｯ繧ｰ繝ｩ繧ｦ繝ｳ繝峨〒螳壽悄逧・↑繧ｯ繝ｪ繝ｼ繝ｳ繧｢繝・・繧貞ｮ溯｡・
        _ = Task.Run(async () =>
        {
            while (true)
            {
                try
                {
                    ...
                    await Task.Delay(TimeSpan.FromMinutes(1)); // 5蛻・＃縺ｨ縺ｫ繧ｯ繝ｪ繝ｼ繝ｳ繧｢繝・・
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "Error in HLS cache cleanup: {Message}", ex.Message);
                    await Task.Delay(TimeSpan.FromMinutes(10)); // 繧ｨ繝ｩ繝ｼ譎ゅ・10蛻・ｾ・▽
                }
            }
        });
