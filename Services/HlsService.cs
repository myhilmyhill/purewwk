...
public class HlsService
{
    ...
    public HlsService(IConfiguration configuration, LuceneService luceneService, IHttpContextAccessor httpContextAccessor)
    {
    ...
    public async Task<string> GenerateHlsPlaylist(string id, int[] bitRates, string? audioTrack)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), "hls_segments", id.Replace("/", "_").Replace("\\", "_"));
        ...
        if (bitRates.Length > 0)
        {
            ...
            ffmpegArgs = $"-y -v error -i \"{fullPath}\" -c:v libx264 -b:v {bitRate}k -c:a aac -f hls -hls_time 10 -hls_list_size 0 -hls_segment_filename \"{tempDir}/segment_%03d.ts\" \"{playlistPath}\"";
            Console.WriteLine($"Using single bitrate: {bitRate}k");
        }
        else
        {
            ffmpegArgs = $"-y -v error -i \"{fullPath}\" -c:v libx264 -c:a aac -f hls -hls_time 10 -hls_list_size 0 -hls_segment_filename \"{tempDir}/segment_%03d.ts\" \"{playlistPath}\"";
        ...
        // Clean up temp dir after some time, but for now leave it
